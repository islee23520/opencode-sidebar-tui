...
const DEFAULT_AIDER_COMMAND = "aider";
...
export class AiderAdapter implements CliAdapter {
  ...
  writeInput(instanceId: string, data: string): void {
    ...
    if (!terminal) {
      this.onError(
        instanceId,
        new Error(`Aider instance not found: ${instanceId}`),
      );
      return;
    }
  ...
  private buildStartCommand(command: string, args?: string[]): string {
    const extractedCommand = this.extractAiderCommand(command);
  ...
  private extractAiderCommand(command: string): string {
    const trimmed = command.trim();
    if (!trimmed) {
      return DEFAULT_AIDER_COMMAND;
    }

    const segments = trimmed.split(/\s+/);
    const aiderIndex = segments.findIndex((segment) =>
      /(^|[\\/])aider$/.test(segment),
    );
    ...
    return segments.slice(aiderIndex).join(" ");
  }
