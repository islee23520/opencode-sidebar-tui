...
export class CodexAdapter implements CliAdapter {
  ...
  writeInput(instanceId: string, data: string): void {
    ...
    if (!terminal) {
      this.onError(
        instanceId,
        new Error(`Codex instance not found: ${instanceId}`),
      );
      return;
    }
  ...
  private buildStartCommand(command: string, args?: string[]): string {
    const extractedCommand = this.extractCodexCommand(command);
  ...
  private extractCodexCommand(command: string): string {
    ...
    return segments.slice(codexIndex).join(" ");
  }
