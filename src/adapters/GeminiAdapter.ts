...
export class GeminiAdapter implements CliAdapter {
  ...
  writeInput(instanceId: string, data: string): void {
    ...
    if (!terminal) {
      this.onError(
        instanceId,
        new Error(`Gemini instance not found: ${instanceId}`),
      );
      return;
    }
  ...
  private buildStartCommand(command: string, args?: string[]): string {
    const extractedCommand = this.extractGeminiCommand(command);
  ...
  private extractGeminiCommand(command: string): string {
    ...
    const geminiIndex = segments.findIndex((segment) =>
      /(^|[\\/])gemini$/.test(segment),
    );
    ...
    return segments.slice(geminiIndex).join(" ");
  }
