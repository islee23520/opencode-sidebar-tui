 ...
 * Handles lifecycle management for Kimi terminal instances
 */
export class KimiAdapter implements CliAdapter {
  ...
  writeInput(instanceId: string, data: string): void {
    ...
    if (!terminal) {
      this.onError(
        instanceId,
        new Error(`Kimi instance not found: ${instanceId}`),
      );
      return;
    }
  ...
  private buildStartCommand(command: string, args?: string[]): string {
    const extractedCommand = this.extractKimiCommand(command);
  ...
  private extractKimiCommand(command: string): string {
    const trimmed = command.trim();
    if (!trimmed) {
      return DEFAULT_KIMI_COMMAND;
    }

    const segments = trimmed.split(/\s+/);
    const kimiIndex = segments.findIndex((segment) =>
      /(^|[\\/])kimi$/.test(segment),
    );
    ...
    return segments.slice(kimiIndex).join(" ");
  }
